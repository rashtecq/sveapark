...
const ScannerView: React.FC = () => {
  ...
  const processImage = async (base64: string) => {
    ...
    try {
      ...
      setError(err.message || "AI could not interpret the sign. Ensure it's well-lit and clearly visible.");
      ...
      <div className="bg-white rounded-t-[32px] -mt-8 relative z-10 p-6 shadow-2xl min-h-[180px]">
        ...
        {result && (
          <div className="animate-in slide-in-from-bottom duration-300">
            <div className={`flex items-start gap-4 p-4 rounded-2xl mb-4 ${result.status === 'ALLOWED' ? 'bg-green-50' :
                result.status === 'FORBIDDEN' ? 'bg-red-50' : 'bg-yellow-50'
              }`}>
              <div className={`p-2 rounded-lg ${result.status === 'ALLOWED' ? 'text-green-600' :
